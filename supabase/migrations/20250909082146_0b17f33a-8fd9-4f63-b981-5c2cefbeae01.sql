-- Secure integrations table: prevent token leakage while preserving team access to non-sensitive fields (fixed column REVOKE syntax)
BEGIN;
...
REVOKE SELECT (access_token) ON TABLE public.integrations FROM PUBLIC, anon, authenticated;
REVOKE SELECT (refresh_token) ON TABLE public.integrations FROM PUBLIC, anon, authenticated;
